...
import { getProduct, formatPrice, formatCompareAt, getActiveProducts, onShopifyDataReady, isShopifyLoading } from "@/lib/products";
...
import { Star, Heart, ShoppingBag, Truck, RefreshCw, Shield, ChevronDown, Share2, X, Zap } from "lucide-react";
  ...
  head: (ctx: any) => {
    ...
    return {
      meta: [
        ...
        { name: 'description', content: product ? product.summary : 'Discover premium modest fashion at LAAF' },
      ...
      <div className="flex gap-0.5">
        {Array.from({ length: 5 }, (_, i) => (
          <Star
            ...
            className={cn(
              "h-4 w-4",
              i < Math.floor(rating) ? "fill-[#f5a623] text-[#f5a623]" : "fill-none text-muted-foreground/30"
            )}
            strokeWidth={1}
          />
        ))}
      </div>
      <span className="text-[0.78rem] text-muted-foreground">{rating} ({count} reviews)</span>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <button type="button" onClick={onClose} className="p-1 hover:bg-muted transition-colors" aria-label="Close">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-6">
          <div>
            <h3 className="text-[0.75rem] font-semibold uppercase tracking-wider text-foreground mb-2">Find Your Abaya Length (By Height)</h3>
            <p className="text-[0.72rem] text-muted-foreground mb-3">Measure from the top of your shoulder down to your feet to find the perfect length. Choose a longer length if you plan to wear heels.</p>
            <table className="w-full text-[0.72rem] border-collapse">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="py-2 text-left font-semibold text-foreground">Recommended Length</th>
                </tr>
              </thead>
              <tbody>
                {[
          ...
          <div className="border-t border-border pt-4">
            <p className="text-[0.72rem] text-muted-foreground leading-relaxed">
              All LAAF abayas are <span className="font-medium text-foreground">one-size-fits-all</span> with a relaxed, loose silhouette designed for modesty. Select your preferred length above — no separate chest or waist sizing is required.
            </p>
          </div>
        </div>

        <p className="mt-5 text-[0.68rem] text-muted-foreground border-t border-border pt-3">
          For custom lengths or bespoke fitting advice, please message our styling concierge on WhatsApp.
        </p>
      </div>
    </div>
  );
...
function ProductPage() {
  ...
  const [reviewStats, setReviewStats] = useState<ReviewStats>({ average: 0, count: 0, breakdown: [0, 0, 0, 0, 0] });
  ...
  if (!product) {
    return (
      <Shell className="py-20">
        ...
        <Link to="/" className="mt-4 inline-block text-sm underline hover:text-accent">Go home</Link>
      </Shell>
    );
  }
  ...
  const related = getActiveProducts().filter((p) => p.slug !== product.slug && p.collections.some((c) => product.collections.includes(c))).slice(0, 4);
  const lengthOptions = product.shopifyVariants
    ?.flatMap((variant) => {
      const option = variant.selectedOptions.find((selected) => selected.name.toLowerCase().includes("length"));
      if (option) return [{ value: option.value.replace(/"/g, ""), availableForSale: variant.availableForSale }];
      const titleMatch = variant.title.match(/\b(50|52|54|56|58|60)\s*"?/);
      return titleMatch ? [{ value: titleMatch[1]!, availableForSale: variant.availableForSale }] : [];
    })
    .filter((option, index, options) => options.findIndex((candidate) => candidate.value === option.value) === index) ?? [];
  const hasAvailableVariant = product.shopifyVariants?.some((variant) => variant.availableForSale) ?? false;
  ...
  const buyNow = useCallback(async () => {
    ...
    const existingIdx = bag.findIndex(
      (i) => i.slug === product.slug && i.size === "One Size" && i.length === length,
    );
    const bagItems = (
      existingIdx >= 0
        ? bag.map((item, idx) => (idx === existingIdx ? { ...item, qty: item.qty + qty } : item))
        : [...bag, { slug: product.slug, size: "One Size", length, qty }]
    ) as { slug: string; size: string; length?: string; qty: number }[];

    const syncItems = bagItems.map((item) => ({
      slug: item.slug,
      size: item.size,
    ...
    try {
      ...
      await syncBagToShopifyCart(syncItems);
  ...
  }, [product, length, qty, addToBag, hasAvailableVariant, bag]);
      ...
      <div className="border-b border-border bg-white py-3">
        <Shell>
          <nav className="text-[0.65rem] text-muted-foreground uppercase tracking-wider">
            ...
            <a href="/collection" className="hover:text-foreground transition-colors">Shop</a>
            <span className="mx-2">/</span>
          ...
          <div className="flex flex-col gap-8 lg:gap-12 lg:flex-row">
            ...
            <div className="flex flex-col lg:w-[440px] lg:shrink-0">
              ...
              <div className="mt-4 flex items-baseline gap-3 flex-wrap">
                <span className="text-[1.4rem] font-semibold text-foreground">{formatPrice(product)}</span>
                {compareAt && (
                  <span className="text-[0.95rem] text-muted-foreground line-through">{compareAt}</span>
                )}
                {discount && (
              ...
              {/* Summary */}
              <p className="mt-4 text-[0.85rem] leading-relaxed text-muted-foreground">{product.summary}</p>

              {/* Colour */}
              <div className="mt-5">
                <p className="text-[0.7rem] font-semibold uppercase tracking-wide text-foreground mb-2">
                  Colour: <span className="font-normal text-muted-foreground">{product.colour}</span>
                </p>
              </div>
              ...
              {product.category === "abayas" && (
                <div className="mt-4">
                  ...
                  {lengthError && (
                    <p className="text-[0.65rem] text-sale mt-1.5">{lengthError}</p>
                  )}
                  {lengthOptions.length === 0 && (
                    <p className="text-[0.65rem] text-sale mt-1.5">No length variants are available for this product.</p>
                  )}
                </div>
              )}
              ...
              <div className="mt-3 flex gap-3">
                ...
                >
                  <Heart className={cn("h-4 w-4", wished && "fill-sale text-sale")} strokeWidth={1.5} />
              ...
              <div className="mt-4 text-center">
                <p className="text-[0.72rem] text-muted-foreground mb-1.5">Need help choosing your length?</p>
                <a
              ...
              <div className="mt-6 grid grid-cols-3 gap-3 border-y border-border py-5">
                {[
                  ...
                  <div key={text} className="flex flex-col items-center text-center gap-1.5">
                    ...
                    <span className="text-[0.6rem] text-muted-foreground leading-tight">{text}</span>
                  </div>
                ))}
              </div>
              ...
              {/* Accordions */}
              <div className="mt-2">
                <AccordionItem title="Product Details" isOpen={activeAccordion === 0} onToggle={() => setActiveAccordion(activeAccordion === 0 ? null : 0)}>
                  <ul className="list-disc pl-4 space-y-1.5">
                    {product.design.map((d) => (
                      <li key={d}>{d}</li>
                    ))}
                  </ul>
                  <p className="mt-3 font-medium text-[0.78rem]">Fabric: <span className="font-normal text-muted-foreground">Premium Nida/Crepe (Opaque, lightweight, non-see-through, luxury drape)</span></p>
                </AccordionItem>
                <AccordionItem title="Shipping & Delivery" isOpen={activeAccordion === 1} onToggle={() => setActiveAccordion(activeAccordion === 1 ? null : 1)}>
                  <p>We carefully prepare every LAAF order for dispatch. Delivery details and applicable shipping charges are shown during checkout before you place your order.</p>
                  <p className="mt-2">For any help with your order or delivery, contact LAAF through WhatsApp.</p>
                </AccordionItem>
                <AccordionItem title="Returns & Exchanges" isOpen={activeAccordion === 2} onToggle={() => setActiveAccordion(activeAccordion === 2 ? null : 2)}>
                  <p>Easy 7-day returns. Item must be unused, unwashed, and in original packaging with tags intact. Exchange or full refund available.</p>
                </AccordionItem>
                <AccordionItem title="Care Instructions" isOpen={activeAccordion === 3} onToggle={() => setActiveAccordion(activeAccordion === 3 ? null : 3)}>
                  <p className="text-[0.72rem] text-muted-foreground italic">Care details for this product will be available soon.</p>
                </AccordionItem>
              </div>
            </div>
          </div>
