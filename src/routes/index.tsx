...
import { getActiveProducts, getActiveCollections, onShopifyDataReady, isShopifyLoading } from "@/lib/products";
  ...
  return (
    <>
        ...
        <Shell>
          <div className="grid grid-cols-3 divide-x divide-border">
            {trustBadges.map(({ icon: Icon, title, sub }) => (
              <div key={title} className="flex min-w-0 flex-col items-center gap-1.5 px-1.5 py-4 text-center sm:px-4 md:gap-2 md:py-6 md:px-6">
                <Icon className="h-5 w-5 shrink-0 text-foreground md:h-7 md:w-7" strokeWidth={1.25} />
                <div className="mt-0.5 min-w-0 space-y-1 md:mt-1.5 md:space-y-1.5">
                  <p className="text-[0.56rem] font-bold uppercase leading-tight tracking-[0.08em] text-foreground sm:text-[0.65rem] md:text-[0.8rem] md:tracking-widest">{title}</p>
                  <p className="text-[0.6rem] leading-tight text-muted-foreground sm:text-[0.7rem] md:text-[0.8rem]">{sub}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Shell>
          <Reveal>
            <div className="flex items-end justify-between mb-5">
              <div>
                <span className="text-[0.65rem] font-semibold uppercase tracking-[0.15em] text-accent">New Season</span>
                <h2 className="font-serif text-[1.8rem] md:text-[2.4rem] font-normal mt-1 text-foreground">New Arrivals</h2>
              </div>
              <a href="/collection?fresh=1" className="hidden sm:inline-flex items-center text-[0.72rem] font-medium uppercase tracking-wide text-foreground border-b border-foreground pb-0.5 hover:text-accent hover:border-accent transition-colors">
                View All →
              </a>
            </div>
          </Reveal>
          ...
          <div className="mt-5 text-center sm:hidden">
            <a href="/collection?fresh=1" className="inline-flex items-center text-[0.72rem] font-medium uppercase tracking-wide text-foreground border-b border-foreground pb-0.5 hover:text-accent hover:border-accent transition-colors">
      ...
      <EditorialBrandSection
        eyebrow="LAAF"
        heading="Quietly Refined"
        description="The modern edit."
        mainImage={{ src: lrgeimge, alt: "LAAF premium abaya editorial, elegant modest fashion", width: 1440, height: 1808 }}
        secondaryImage={{ src: detailBlack, alt: "LAAF black abaya sleeve detail with ruffled cuffs", width: 1008, height: 1008 }}
        ctaLabel="Shop"
        ctaUrl="/collection"
      />
        ...
        <Shell>
          <Reveal>
            <div className="flex flex-col items-center text-center mb-8">
              <span className="text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-accent">Explore LAAF</span>
              <h2 className="font-serif text-[2rem] md:text-[2.8rem] font-normal mt-2 text-foreground">The Collections</h2>
            </div>
          </Reveal>
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-2 md:gap-4 lg:grid-cols-12">
            ...
            {collectionsList.slice(0, 2).map((col, i) => (
              <Reveal key={col.slug} delay={i * 100} className="min-h-0 overflow-hidden rounded-none aspect-[4/5] lg:col-span-6 lg:aspect-auto lg:h-[500px]">
                <Link to={`/collections/${col.slug}`} className="group relative flex h-full w-full overflow-hidden bg-secondary">
                  <img
                    src={col.heroImage}
                    alt=""
                    aria-hidden="true"
                    className="absolute inset-0 h-full w-full object-cover blur-[2px] scale-105 transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-110"
                  />
                  <img
                    src={col.heroImage}
                    alt={col.title}
                    className="absolute inset-0 h-full w-full object-contain transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-105"
                  />
                  ...
                  <div className="absolute inset-x-0 bottom-0 z-10 p-3 text-left sm:p-6 md:p-8">
                    <h3 className="font-serif text-[0.72rem] leading-tight tracking-wide text-white sm:text-[1.1rem] md:text-[2.2rem]">{col.title}</h3>
                  </div>
                </Link>
              </Reveal>
            ))}
            ...
            {collectionsList.slice(2, 4).map((col, i) => (
              <Reveal key={col.slug} delay={(i + 2) * 100} className="min-h-0 overflow-hidden rounded-none aspect-[4/5] lg:col-span-6 lg:aspect-auto lg:h-[450px]">
                <Link to={`/collections/${col.slug}`} className="group relative flex h-full w-full overflow-hidden bg-secondary">
                  <img
                    src={col.heroImage}
                    alt=""
                  ...
                  <div className="absolute inset-x-0 bottom-0 z-10 p-2.5 text-left sm:p-4 md:p-6">
                    <h3 className="font-serif text-[0.68rem] leading-tight tracking-wide text-white sm:text-[0.95rem] md:text-[1.8rem]">{col.title}</h3>
                  </div>
                </Link>
              </Reveal>
            ))}
            ...
            {collectionsList.slice(4, 5).map((col) => (
              <Reveal key={col.slug} delay={400} className="min-h-0 overflow-hidden rounded-none col-span-2 h-[200px] collection-card-fifth lg:col-span-12 lg:h-[450px]">
                <Link to={`/collections/${col.slug}`} className="group relative flex h-full w-full overflow-hidden bg-secondary">
                  <img
                    src={col.heroImage}
                    alt=""
        ...
        <div className="relative z-10 flex h-full items-center">
          <Shell>
            <Reveal className="max-w-lg text-white">
              <span className="text-[0.65rem] font-medium uppercase tracking-[0.2em] text-white/70">Craftsmanship</span>
              <h2 className="font-serif text-[2rem] md:text-[2.8rem] font-normal leading-[1.1] mt-3">
                Elegance lies in<br />the details.
              </h2>
              <p className="mt-4 text-[0.85rem] leading-relaxed text-white/80 max-w-sm">
                Every stitch tells a story of artisanal craft, premium fabrics, and contemporary design.
              </p>
              <a
                href="/about"
        ...
        <Shell>
          <Reveal>
            <div className="text-center mb-6">
              <span className="text-[0.65rem] font-semibold uppercase tracking-[0.15em] text-accent">Reviews</span>
              <h2 className="font-serif text-[1.8rem] md:text-[2.4rem] font-normal mt-1 text-foreground">What Our Customers Say</h2>
            </div>
          </Reveal>
            ...
            {testimonials.map((t, i) => (
              <Reveal key={t.name} delay={i * 80}>
                <div className="bg-white p-6 border border-border">
                  ...
                  <p className="mt-3 text-[0.85rem] leading-relaxed text-foreground/80">&ldquo;{t.text}&rdquo;</p>
                  <div className="mt-4 pt-4 border-t border-border">
                    <p className="text-[0.75rem] font-semibold text-foreground">{t.name}</p>
                    <p className="text-[0.65rem] text-muted-foreground">Verified Buyer — {t.product}</p>
                  </div>
                </div>
              </Reveal>
            ))}
